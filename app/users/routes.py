...
def view_user(user_id):
    ...
    # --- FUNCIONES DE FORMATO PARA TAREAS DEL PERFIL ---
    def format_venezuelan_id(id_str):
        if not id_str: return 'N/A'
        id_clean = str(id_str).strip().upper().replace('-', '')
        # Verificamos si ya empieza con alguna letra común, si no, forzamos la V
    ...
    if person:
        # 1. Formateo y extracción de la data personal del usuario
        ...
        user.formatted_phone = format_venezuelan_phone(getattr(person, 'phone', ''))
        
        # Intentamos obtener la ubicación personal del usuario en cascada
        user.personal_state = 'N/A'
        user.personal_municipality = 'N/A'
        user.personal_parish = 'N/A'
        user.personal_city = 'N/A'
        
        if getattr(person, 'parish', None):
            user.personal_parish = person.parish.name
            if getattr(person.parish, 'municipality', None):
                user.personal_municipality = person.parish.municipality.name
                if getattr(person.parish.municipality, 'state', None):
                    user.personal_state = person.parish.municipality.state.name
            if getattr(person.parish, 'city', None):
                user.personal_city = person.parish.city.name
        ...
        if inst_staff:
            ...
            # Extracción segura de la ciudad de la institución
            city_name = 'N/A'
            if inst and getattr(inst, 'parish', None) and getattr(inst.parish, 'city', None):
                city_name = inst.parish.city.name
            
            corp_data = {
                'id_card': inst.institution_code if inst else 'N/A', # Mantenemos el código de plantel
                ...
                'phone_main': format_venezuelan_phone(inst.phone) if inst else 'N/A', # Filtro de teléfono
                'phone_sec': 'N/A',
                ...
                'city': city_name, # Incluimos la ciudad
            ...
            if comp_staff:
                ...
                city_name = 'N/A'
                if place and getattr(place, 'parish', None) and getattr(place.parish, 'city', None):
                    city_name = place.parish.city.name
                ...
                if comp:
                    corp_data = {
                        ...
                        'dependency': 'N/A',
                        'position': pos.name if pos else 'N/A',
                        'phone_main': format_venezuelan_phone(place.phone) if place else 'N/A', # Filtro de teléfono
                        'phone_sec': 'N/A',
